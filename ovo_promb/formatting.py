...
def print_nearest(queries: list[str], hit_lists: list[list[str]], wrap=10, file=sys.stdout):
    assert len(queries) == len(
        hit_lists), f"Expected same number of positions in query list and hit list, got {len(queries)} != {len(hit_lists)}"
    if not queries:
        return
    ...
    for query_batch, hits_batch in zip(chunk_list(queries, wrap), chunk_list(hit_lists, wrap)):
        ...
        for query, hits in zip(query_batch, hits_batch):
            ...
            for hit, hit_line, mut_line in zip(hits, hit_lines, mut_lines):
                ...
                mut_line.append(
                    ''.join(('+' if are_similar(aa, bb) else '^') if aa != bb else ' ' for aa, bb in zip(query, hit)))
...
def chop_seq_peptides(seq: str, k: int):
    """Return all overlapping k-mer peptides from sequence"""
